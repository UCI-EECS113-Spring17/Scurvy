...
uint32_t check_message(XGpio *gpio)
{
    return (XGpio_DiscreteRead(gpio, 1) >> 2) & 0x01;
}
...
int main() {
    ...
    XGpio interrupt;
    XGpio_Initialize(&interrupt, XPAR_GPIO_0_DEVICE_ID);
    XGpio_SetDataDirection(&interrupt, 1, 0xFF);
    ...
    while(1) {
        ...
        switch(command) {
            ...
            case SEND_MESSAGE:
                ...
                break;
            case CHECK_MESSAGE:
                MAILBOX_DATA(0) = check_message(&interrupt);
                break;
            // Test Commands
